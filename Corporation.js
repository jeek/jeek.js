...
export class Corporation extends CorpBaseClass {
  ...
  get isHappy() {
    return this.divisionsObj
      .map((division) => division.isHappy)
      .reduce((a, b) => a && b);
  }
  async Start() {
    while ([undefined, false].includes(this.c.hasCorporation())) {
      try {
        this.c.createCorporation(
          this.settings.includes("name") ? this.settings.name : "Corporation",
          this.ns.getPlayer().bitNodeN == 3 ? false : true
        );
  ...
  async Continue() {
    if (
      !Object.keys(this.settings).includes("scam") ||
      this.settings.scam == false
    ) {
      await this.WaitOneLoop();
      for (let i = 1; i <= 4; i++) {
        while (
          this.round == i &&
          this.c.getInvestmentOffer().funds +
            (this.funds > 0 ? this.funds : 0) <
            (Object.keys(this.settings).includes("baseOffers")
              ? this.settings["baseOffers"][i - 1]
              : [210e9, 5e12, 800e12, 500e15]) *
              this.ns.getBitNodeMultipliers().CorporationValuation
        ) {
          await this.WaitOneLoop();
        }
        if (this.round == i) {
      ...
      if (!this.c.getCorporation().public) this.c.goPublic(0);
    }
    while (this.round < 5) await this.WaitOneLoop();
    this.c.issueDividends(1);
    while (this.funds < 1e21) await this.WaitOneLoop();
    this.c
      .getConstants()
      .unlockNames.map((unlock) =>
        this.c.hasUnlockUpgrade(unlock) ? true : this.c.unlockUpgrade(unlock)
      );
  }
  async StartDivision(type, settings = {}) {
    ...
    let plan = "Guide";
    if (
      Object.keys(this.settings).includes(type) &&
      Object.keys(this.settings[type]).includes("plan")
    ) {
      plan = this.settings[type].plan;
    }
    let makesProducts = Object.keys(this.c.getIndustryData(type)).includes(
      "product"
    );
    let makesMaterials = Object.keys(this.c.getIndustryData(type)).includes(
      "producedMaterials"
    );
    switch (plan) {
      case "Scam":
      ...
      case "Guide":
        if (makesMaterials) {
          this.divisionsObj[type] = new GuideMaterial(
            this.ns,
            this,
            type,
            this.settings
        ...
        if (makesProducts) {
          this.divisionsObj[type] = new GuideProduct(
            this.ns,
            this,
            type,
            this.settings
      ...
      default:
        if (makesMaterials) {
          this.divisionsObj[type] = new JeekMaterial(
            this.ns,
            this,
            type,
            this.settings
        ...
        if (makesProducts) {
          this.divisionsObj[type] = new JeekProduct(
            this.ns,
            this,
            type,
            this.settings
  ...
  async GetUpgrade(upgrade, level = 1) {
    while (this.c.getUpgradeLevel(upgrade) < level) {
      while (
        this.c.getUpgradeLevel(upgrade) < level &&
        this.c.getUpgradeLevelCost(upgrade) <= this.funds
      ) {
