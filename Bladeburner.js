...
export class Bladeburner {
  constructor(ns, Game, settings = {}) {
    ...
    this.raid = Object.keys(this.settings).includes("raid")
      ? this.settings.raid
      : true;
    this.sting = Object.keys(this.settings).includes("sting")
      ? this.settings.string
      : true;
    this.maxChaos = Object.keys(this.settings).includes("maxChaos")
      ? this.settings.maxChaos
      : 30;
    this.minStamina = Object.keys(this.settings).includes("minStamina")
      ? this.settings.minStamina
      : 0.6;
    this.maxStamina = Object.keys(this.settings).includes("maxStamina")
      ? this.settings.maxStamina
      : 0.9;
    this.Game = Game ? Game : new WholeGame(ns);
    this.log = ns.tprint.bind(ns);
    if (ns.flags(cmdlineflags)["logbox"]) {
      this.log =
        this.Game.sidebar.querySelector(".bladebox") ||
        this.Game.createSidebarItem("Bladeburner", "", "B", "bladebox");
    ...
    (async () => {
      ...
      (await Do(this.ns, "ns.bladeburner.getBlackOpNames")).forEach(
        (x) => (this.bbTypes[x] = "Black Op")
      );
    })();
    (async () => {
      while (!(await Do(this.ns, "ns.bladeburner.joinBladeburnerDivision"))) {
      ...
      (await Do(this.ns, "ns.bladeburner.getOperationNames")).forEach(
        (x) => (this.bbTypes[x] = "Operation")
      );
    })();
    (async () => {
      while (!(await Do(this.ns, "ns.bladeburner.joinBladeburnerDivision"))) {
      ...
      (await Do(this.ns, "ns.bladeburner.getContractNames")).forEach(
        (x) => (this.bbTypes[x] = "Contract")
      );
    })();
  }
  get chaosHere() {
  ...
  async ["getActionAutolevel"](name) {
    while (Object.keys(this.bbTypes).length == 0) await this.ns.asleep(0);
    return await Do(
      this.ns,
  ...
  async UpgradeSkills(count = 1) {
    while (
      await Do(this.ns, "ns.bladeburner.upgradeSkill", "Overclock", count)
    ) {
    ...
    let currentrank = await DoAll(
      this.ns,
      "ns.bladeburner.getSkillLevel",
      Object.keys(skillmods)
    );
    let cost = await DoAll(
      this.ns,
  ...
  async inciteViolence() {
    let city = Object.entries(
      await DoAll(this.ns, "ns.bladeburner.getCityEstimatedPopulation", CITIES)
  ...
  async recoverIfNecessary(lower = -1, upper = -1) {
    ...
    if (
      lower >
      (await Do(this.ns, "ns.bladeburner.getStamina")).reduce((a, b) => a / b)
    ) {
  ...
  async deescalate(goal = -1) {
    ...
    if (
      ...
      (await Do(
        this.ns,
        "ns.bladeburner.getCityChaos",
        await Do(this.ns, "ns.bladeburner.getCity")
      ))
    ) {
  ...
  get operationCount() {
    return (async () => {
      try {
        return (
          (await this.bbOpCount("Investigation")) +
          (await this.bbOpCount("Undercover Operation")) +
          (await this.bbOpCount("Stealth Retirement Operation")) +
          (await this.bbOpCount("Assassination"))
        );
      } catch (e) {
        return 0;
      }
    ...
    return (async () => {
      try {
        return (await Do(this.ns, "ns.bladeburner.getOperationNames"))
          .filter((x) => (this.raid ? true : x != "Raid"))
          .filter((x) => (this.sting ? true : x != "Sting Operation"));
      } catch (e) {
        return [];
      }
    })();
      ...
      try {
        return Object.entries(
          await DoAllComplex(
            ...
            "ns.bladeburner.getActionCountRemaining",
            (
              await Do(this.ns, "ns.bladeburner.getBlackOpNames")
            ).map((x) => ["Black Op", x])
          )
        )
          .map((x) => [x[0].split(",")[1], x[1]])
          .filter((x) => x[1] > 0)[0][0];
      } catch (e) {
        return 0;
      }
  ...
  async createDisplay() {
    ...
    eval("window").listenUp = (message) => {
      globalThis.bbQueue.push(message);
    };
  ...
  async updateDisplay() {
    ...
    answer += "<H1>Rank: " + Math.floor(0.5 + myrank).toString() + "<BR>";
    ...
    answer +=
      ...
      "<FONT COLOR=" +
      this.ns.ui.getTheme()[
        chaos < 40 ? "success" : chaos < 50 ? "warning" : "error"
      ] +
      ">" +
      jFormat(chaos) +
      "</FONT><BR>";
    answer +=
      "Communities: " +
      (
        await Do(this.ns, "ns.bladeburner.getCityCommunities", mycity)
      ).toString() +
      "<BR>";
    answer +=
      "Estimated Population: " +
      jFormat(
        await Do(this.ns, "ns.bladeburner.getCityEstimatedPopulation", mycity)
      ) +
      "</H1></TD><TD><H1>";
    ...
    if (0 < (await this.Game.Sleeves.numSleeves)) {
      ...
      for (let i = 0; i < (await this.Game.Sleeves.numSleeves); i++) {
        try {
          if (
            (await Do(this.ns, "ns.sleeve.getTask", i)).actionName !=
            (await Do(this.ns, "ns.sleeve.getTask", 0)).actionName
          )
            wildcard = false;
        } catch {}
      }
    ...
    for (let city of CITIES.sort()) {
      if (city != mycity)
        answer += td(
          await Do(this.ns, "ns.bladeburner.getCityCommunities", city),
          "RIGHT"
        );
    }
    answer += "</TR>";
    ...
    for (let city of CITIES.sort()) {
      if (city != mycity)
        answer += td(
          jFormat(
            await Do(this.ns, "ns.bladeburner.getCityEstimatedPopulation", city)
          ),
          "RIGHT"
        );
    }
    ...
    if (currentAction.type != "Idle") {
      answer +=
        "<CENTER><H1>" +
        currentAction.type.replace("BlackOp", "Black Op") +
        ": " +
        currentAction.name +
        " " +
        timeFormat(
          ((await Do(
            ...
            (await Do(this.ns, "ns.bladeburner.getActionCurrentTime"))) /
            1000
        ) +
        "</H1></CENTER><BR>";
      answer += "<TABLE WIDTH=100% BORDER=0><TR>";
      let percentage =
        (100 * (await Do(this.ns, "ns.bladeburner.getActionCurrentTime"))) /
        (await Do(
          this.ns,
          "ns.bladeburner.getActionTime",
          currentAction.type,
    ...
    answer +=
      "<TABLE WIDTH=100% BORDER=1><TR><TH>Jobs</TH><TH>Skills " +
      (await Do(this.ns, "ns.bladeburner.getSkillPoints")).toString() +
      "</TH></TR>";
    ...
    for (let op of await Do(this.ns, "ns.bladeburner.getBlackOpNames")) {
      if (
        ...
        answer +=
          "<FONT COLOR=" +
          this.ns.ui.getTheme()[myrank < oprank ? "warning" : "success"] +
          ">" +
          oprank.toString() +
          "</FONT>";
        answer +=
          ...
          "%) " +
          (this.isKillOp(op) ? this.killicon : "") +
          (this.isStealthOp(op) ? this.stealthicon : "") +
          "<BR>";
      }
    }
    answer += "</TD>";
    ...
    for (let skill of await Do(this.ns, "ns.bladeburner.getSkillNames")) {
      if ((await Do(this.ns, "ns.bladeburner.getSkillLevel", skill)) > 0) {
        answer +=
          ...
          ": " +
          (
            await Do(this.ns, "ns.bladeburner.getSkillLevel", skill)
          ).toString() +
          " (" +
          (await Do(this.ns, "ns.bladeburner.getSkillUpgradeCost", skill)) +
          ")<BR>";
      }
    }
    for (let skill of await Do(this.ns, "ns.bladeburner.getSkillNames")) {
