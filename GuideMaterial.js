...
class GuideMaterial extends MaterialIndustry {
  ...
  async Start() {
    ...
    while (
      !this.c
        .getCorporation()
        .divisions.map((x) => this.c.getDivision(x))
        .map((x) => x.type)
        .includes(this.industry)
    ) {
      await this.WaitOneLoop();
    }
    this.Research(["Hi-Tech R&D Laboratory"]).then(
    ...
    this.industryData.producedMaterials.map((material) =>
      this.cities.map((city) => city.w.sellMaterial(material))
    );
    if (this.round <= 1) {
      await this.getHappy();
    }
    ...
    if (
      this.round == 1 ||
      this.round == 3 ||
      this.c.getMaterial(this.name, this.HQ, "AI Cores").qty == 0 ||
      this.c.getMaterial(this.name, this.HQ, "Hardware").qty == 0 ||
      this.c.getMaterial(this.name, this.HQ, "Real Estate").qty == 0 ||
      this.c.getMaterial(this.name, this.HQ, "Robots").qty == 0
    )
    ...
    let redo = false;
    if (
      this.getDivision.research < 2 ||
      this.cities.map((city) => city.o.size).reduce((a, b) => (a > b ? b : a)) <
        9
    ) {
      redo = true;
      this.cities.map((city) =>
    ...
    for (
      let i = 1;
      i <= Math.ceil(10 * this.ns.getBitNodeMultipliers().CorporationValuation);
      i++
    ) {
