...
function helperScripts(ns) {
  ...
  writeIfNotSame(
    ns,
    "/temp/hackstock.js",
    `export async function main(ns) {await ns.hack(ns.args[0], {"stock": true});}`
  );
  writeIfNotSame(
    ns,
    "/temp/grow.js",
  ...
  writeIfNotSame(
    ns,
    "/temp/growstock.js",
    `export async function main(ns) {await ns.grow(ns.args[0], {"stock": true});}`
  );
  writeIfNotSame(
    ns,
    "/temp/weaken.js",
...
export function timeFormat(n) {
  let seconds = n % 60;
  n = Math.floor((n - seconds) / 60 + 0.5);
  let minutes = n % 60;
  n = Math.floor((n - minutes) / 60 + 0.5);
  let hours = n;
...
function finalform(ns) {
  ...
  while (i < lines.length) {
    ...
    if (
      lines[i].includes("await Do") &&
      !lines[i].includes("FFIGNORE") &&
      !lines[i].includes("DoAll")
    ) {
