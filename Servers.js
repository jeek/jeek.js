...
export class Servers {
  constructor(ns, Game) {
    ...
    [
      ...
      "megacorp",
      "fulcrumassets",
    ].map((x) => (this[x] = new Server(ns, x, Game)));
    this.log = ns.tprint.bind(ns);
    if (ns.flags(cmdlineflags)["logbox"]) {
      this.log =
        this.Game.sidebar.querySelector(".servers") ||
        this.Game.createSidebarItem("Servers", "", "S", "servers");
      this.body = this.log.body;
      this.body.innerHTML =
        "<canvas width=1000 height=1000 id='serverbox'></canvas>";
    ...
    if (this.ns.flags(cmdlineflags)["logbox"]) {
        ...
        for (let j = 0; j < layout[i].length; j++) {
          ...
          for (let server of possible) {
            ...
            if (addThis) {
              ...
              if (
                layout[i][j] == "home" &&
                (await Do(this.ns, "ns.scan", server)).length == 1
              ) {
      ...
      while (true) {
        Object.keys(scans).map(
          (x) => (minsec[x] = Do(this.ns, "ns.getServerMinSecurityLevel", x))
        );
        Object.keys(scans).map(
          (x) => (sec[x] = Do(this.ns, "ns.getServerSecurityLevel", x))
        );
        Object.keys(scans).map(
          (x) => (mon[x] = Do(this.ns, "ns.getServerMoneyAvailable", x))
        );
        Object.keys(scans).map(
          (x) => (maxmon[x] = Do(this.ns, "ns.getServerMaxMoney", x))
        );
        ...
        for (let i = 0; i < layout.length; i++) {
          for (let j = 0; j < layout[i].length; j++) {
            ...
            if (layout[i].length > 1) {
              myX =
                (750 / (layout[i].length - 1)) *
                layout[i].findIndex((x) => x === server);
            }
            let myY = heights[i];
            if (i + 1 < layout.length) {
              let connectTo = scans[server].filter((x) =>
                layout[i + 1].includes(x)
              );
              for (let target of connectTo) {
                let theirX = 375;
                if (layout[i + 1].length > 1) {
                  theirX =
                    (750 / (layout[i + 1].length - 1)) *
                    layout[i + 1].findIndex((x) => x === target);
                }
            ...
            ctx = c.getContext("2d");
            let security =
              ((await sec[server]) - (await minsec[server])) /
              (99 - (await minsec[server]));
            ctx.beginPath();
  ...
  async pop_them_all() {
    ...
    for (let server of await this.serverlist) {
      if (
        (await Do(this.ns, "ns.getServer", server)).openPortCount >=
        (await Do(this.ns, "ns.getServerNumPortsRequired", server))
      ) {
  ...
  async buyDubs() {
    ...
    if (
      servers.length == (await Do(this.ns, "ns.getPurchasedServerLimit", ""))
    ) {
      if (maxRam > (await Do(this.ns, "ns.getServerMaxRam", servers[0]))) {
        if (
          (await this.Game.Player.money) >
          (await Do(this.ns, "ns.getPurchasedServerCost", maxRam))
        ) {
    ...
    if (
      ...
      let rams = Object.values(
        await DoAll(this.ns, "ns.getServerMaxRam", servers)
      ).reduce((a, b) => (a > b ? a : b), 4);
      return await Do(
        this.ns,
        "ns.purchaseServer",
        "pserv-" + servers.length.toString(),
        rams * 2 < maxRam ? rams * 2 : maxRam
      );
    }
    return false;
  }
  async display() {
    this["window"] =
      this["window"] || (await makeNewWindow("Servers", this.ns.ui.getTheme()));
    let text =
      "<TABLE CELLPADDING=0 CELLSPACING = 0 BORDER=1><TR><TH>Name</TD><TH>Popped</TD></TR>";
    for (let server of this.serverlist) {
      text +=
        "<TR><TD>" +
        server +
        "</TD><TD ALIGN=CENTER>" +
        ((await Do(this.ns, "ns.hasRootAccess", server)) ? "✅" : "❌") +
        "</TD></TR>";
    }
    text += "</TABLE>";
