...
class ProductIndustry extends Division {
  constructor(ns, Corp, industry, settings = {}) {
    ...
    if (!Object.keys(this.settings).includes("productNames")) {
      this.settings.productNames = ["A", "B", "C", "D", "E"].map(
        (x) => this.industry + " " + x
      );
    }
  }
    ...
    while (true) {
      while (
        this.c.getProduct(
          this.name,
          "Sector-12",
          this.getDivision.products[this.getDivision.products.length - 1]
        ).developmentProgress < 100
      ) {
        await this.WaitOneLoop();
      }
      this.cities.map((city) =>
        Object.keys(city.w.pricing).includes(
          this.c.getProduct(
            this.name,
            city.name,
            this.getDivision.products[this.getDivision.products.length - 1]
      ...
      if (
        this.getDivision.products.length ==
        3 +
          this.c.hasResearched(this.name, "uPgrade: Capacity.I") +
          this.c.hasResearched(this.name, "uPgrade: Capacity.II")
      ) {
        let rats = [];
        for (let product of this.getDivision.products) {
          rats.push([
            this.c.getProduct(this.name, "Sector-12", product).rat,
            product,
          ]);
        }
      ...
      while (!done) {
        try {
          this.c.makeProduct(
            this.name,
            this.HQ,
            this.settings.productNames.filter(
              (x) => !this.getDivision.products.includes(x)
            )[0],
            Math.floor(this.funds / 2.1),
            Math.floor(this.funds / 2.1)
          );
          done = true;
        } catch {
