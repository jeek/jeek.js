...
export class WholeGame {
    ...
    if (ns.flags(cmdlineflags)["logbox"]) {
      ...
      if (!this.sidebar) {
        ...
        this.sidebar = this.doc.body.appendChild(
          this.elemFromHTML(
            `<div class="sb"><style>${this.css}</style><div class="head"><a class="icon collapser">\ueab6</a><span class=title>box.sidebar v1.1j</span></div>`
          )
        );
        ...
        this.sidebar.querySelector(".head").addEventListener("click", () => {
          ...
          setTimeout(
            () =>
              ...
              Object.assign(this.doc.querySelector(".monaco-editor").style, {
                width: "0px",
              }),
            255
          );
        ...
        this.win._boxEdgeDetect = () =>
          this.doc
            .querySelectorAll(".sb .box")
            .forEach((box) =>
              Object.assign(box.style, {
                left:
                  Math.max(
                    Math.min(
                      this.win.innerWidth - box.offsetWidth,
                      box.offsetLeft
                    ),
                    0
                  ) + "px",
                top:
                  Math.max(
                    Math.min(
                      this.win.innerHeight - box.offsetHeight,
                      box.offsetTop
                    ),
                    0
                  ) + "px",
    ...
    item.head
      ...
      .addEventListener(
        ...
        (e) =>
          item.classList.contains("box") &&
          this.transition(
            () => item.classList.toggle("c") || this.win._boxEdgeDetect()
          )
      );
    item.head.addEventListener(
      "contextmenu",
      (e) => e.preventDefault() || this.contextMenu(item, e.clientX, e.clientY)
    );
    Object.assign(item.style, {
  ...
  get hours() {
    return (async () => {
      try {
        return Math.floor(
          (await Do(this.ns, "ns.getPlayer")).playtimeSinceLastAug / 3600000
        );
      } catch (e) {
        return 1;
      }
  ...
  async SoftReset() {
    writeIfNotSame(
      this.ns,
      "/temp/restart.js",
      "export async function main(ns) {ns.spawn('jeek.js', 1, \"" +
        this.ns.args.join('","') +
        '");}'
    );
