...
export async function bn7(Game) {
  ...
  if (!(await Game.Bladeburner.start())) return false;
  Game.Bladeburner.log("Start.");
  while (true) {
    let zc = 1;
    while (await Game.Bladeburner.UpgradeSkills(zc)) zc += 1;
    await Game.Sleeves.bbEverybody("Field analysis");
    await Game.Bladeburner.hardStop();
    while (
      (await Game.Bladeburner.contractCount) +
        (await Game.Bladeburner.operationCount) >
      0
    ) {
      Game.Hacknet.goal =
        1000 > (await Game.Bladeburner.skillPoints)
      ...
      if (
        (await Game.Bladeburner.rank) >= 25 &&
        !(await Game.Player.factions).includes("Bladeburners")
      )
      ...
      for (let city of CITIES) {
        ...
        for (let action of (await Game.Bladeburner.opNames).concat(
          await Game.Bladeburner.contractNames
        )) {
          if ((await Game.Bladeburner.bbActionCount(action)) > 0) {
            ...
            for (
              ...
              if (
                Game.Bladeburner.bbTypes[action] == "Contract" ||
                (await Game.Bladeburner.getChance(action))[0] > 0.95
              )
                best.push([
                  level,
                  Game.Bladeburner.bbTypes[action],
                  action,
                  city,
                  (await Game.Bladeburner.bbActionCount(action)) *
                    (((await Game.Bladeburner.getChance(action)).reduce(
                      (a, b) => (a + b) / 2
                    ) *
                      (await Game.Bladeburner.repGain(action, level))) /
                      (await Game.Bladeburner.bbActionTime(action))),
                ]);
            }
      ...
      best = best.sort((a, b) => {
        if (a[2] == "Assassination" && b[2] != "Assassination") return 1;
        if (a[2] != "Assassination" && b[2] == "Assassination") return -1;
        if (a[1] == "Operation" && b[1] != "Operation") return 1;
        if (a[1] != "Operation" && b[1] == "Operation") return -1;
        return 0;
      });
      ...
      if (nextBlackOp != "0" && nextBlackOp != 0) {
        if (
          (await Game.Bladeburner.rank) >=
          (await Game.Bladeburner.blackOpRank(nextBlackOp))
        ) {
          if (
            (await Game.Bladeburner.successChance("Operation Ultron"))[0] > 0.99
          ) {
            if (
              (await Game.Bladeburner.successChance(nextBlackOp))[0] >
              ([
                "Operation Centurion",
                "Operation Vindictus",
                "Operation Daedalus",
              ].includes(nextBlackOp)
                ? 0.2
                : 0.99)
            ) {
      ...
      if (best[best.length - 1][1] != "Black Op") {
        await Game.Bladeburner.setAutoLevel(
          best[best.length - 1][2],
          1e6 < (await Game.Bladeburner.rank)
        );
      ...
      if (best[best.length - 1][1] != "Black Op") {
        await Game.Bladeburner.setLevel(
          best[best.length - 1][2],
          best[best.length - 1][0]
        );
      }
      await Game.Bladeburner.hardStop();
      if (best[best.length - 1][1] == "Black Op") {
      ...
      if (best[best.length - 1][1] != "Black Op") {
        ...
        if ((await Game.Bladeburner.actionCount("Retirement")) >= 30) {
          await Game.Sleeves.bbDo(
            shox[cur],
            "Take on contracts",
            best.filter((x) => x[2] == "Retirement").reverse()[0][2]
          );
          cur += 1;
        }
        if ((await Game.Bladeburner.actionCount("Bounty Hunter")) >= 30) {
          await Game.Sleeves.bbDo(
            shox[cur],
            "Take on contracts",
            best.filter((x) => x[2] == "Bounty Hunter").reverse()[0][2]
          );
          cur += 1;
        }
        if ((await Game.Bladeburner.actionCount("Tracking")) >= 100) {
          await Game.Sleeves.bbDo(
            shox[cur],
            "Take on contracts",
            best.filter((x) => x[2] == "Tracking").reverse()[0][2]
          );
          cur += 1;
        }
        if (shox.length > cur) {
      ...
      while (
        (await Game.Bladeburner.currentAction).type != "Idle" &&
        Game.Bladeburner.minStamina <
          (await Game.Bladeburner.stamina).reduce((a, b) => a / b) &&
        (await Game.Bladeburner.actionCount(best[best.length - 1][2])) > 0
      ) {
        if (
          best[best.length - 1][0] == "Black Op" &&
          0.2 > (await Game.Bladeburner.successChance(nextBlackOp))[0]
        )
          break;
        await Game.Sleeves.bbCombatAugs();
        ...
        Game.Hacknet.goal =
          1000 > (await Game.Bladeburner.skillPoints)
            ? "Exchange for Bladeburner SP"
            : "Generate Coding Contract";
        if (0.999 < (await Game.Bladeburner.successChance(nextBlackOp))) break;
        if (
          best[best.length - 1][0] <
          (await Game.Bladeburner.actionMaxLevel(best[best.length - 1][2]))
        ) {
          if (
            1 ==
            (await Game.Bladeburner.successChance(best[best.length - 1][2]))[0]
          ) {
        ...
        if (
          10 + (await Game.Bladeburner.cityChaos) <=
          (await Game.Bladeburner.chaosHere)
        )
          break;
        await Game.ns.asleep(1000);
      }
