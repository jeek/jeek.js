...
export class Sleeves {
  constructor(ns, Game) {
    ...
    if (this.ns.flags(cmdlineflags)["logbox"]) {
      this.log =
        this.Game.sidebar.querySelector(".sleevelog") ||
        this.Game.createSidebarItem("Sleeves", "", "S", "sleevelog");
      this.display = this.Game.sidebar
        .querySelector(".sleevelog")
  ...
  get numSleeves() {
    return (async () => {
      try {
        if (
          (await Do(this.ns, "ns.singularity.getOwnedSourceFiles")).filter(
            (x) => x.n == 10
          ).length > 0
        )
  ...
  async SleeveInfoLog() {
    while (true) {
      let result =
        "<TABLE BORDER=1 CELLSPACING=0 WIDTH=100% CELLPADDING=0><TR><th>id</th><th>hack</th><th>str</th><th>def</th><th>dex</th><th>agi</th><th>int</th><th>shk</th></tr>";
      if (this.ns.flags(cmdlineflags)["logbox"]) {
        let rowData = {};
        for (let i = 0; i < (await this.numSleeves); i++) {
          ...
          try {
            task = Object.values(
              await Do(this.ns, "ns.sleeve.getTask", i)
            ).join(" / ");
          } catch {}
          let thisRow =
            "<TD ALIGN=CENTER>" +
            [
              me.skills.hacking.toString(),
              me.skills.strength.toString(),
              me.skills.defense.toString(),
              me.skills.dexterity.toString(),
              me.skills.agility.toString(),
              me.skills.intelligence.toString(),
              Math.ceil(me.shock).toString(),
        ...
        let rowSort = Object.keys(rowData).sort(
          (a, b) => rowData[a][0] - rowData[b][0]
        );
  ...
  async trainWithMe(stat) {
    let sleeves = Array(await this.numSleeves)
      .fill(0)
      .map((x, i) => i);
    let sleeveData = {};
    let totalshock = 0;
  ...
  async startAGangFirst() {
        ...
        for (let i = 0; i < (await this.numSleeves); i++) {
          if (
            ...
            (await Do(
              ...
              await Do(this.ns, "ns.sleeve.getSleeve", i),
              "Mug"
            ))
          ) {
            let bestGym = {};
            ...
            for (let stat of ["Strength", "Dexterity", "Defense", "Agility"]) {
              ...
              bestGym[stat]["exp"][stat.toLowerCase()] +=
                200 * bestGym[stat]["mults"][stat.toLowerCase() + "_exp"];
              for (let stat2 of [
                "Strength",
                "Dexterity",
                "Defense",
                ...
                bestGym[stat]["skills"][stat2.toLowerCase()] =
                  Math.max(
                    bestGym[stat]["mults"][stat2.toLowerCase()] *
                      (32 *
                        Math.log(
                          bestGym[stat]["exp"][stat2.toLowerCase()] + 534.5
                        ) -
                        200),
                    1
                  ) * mults[stat + "LevelMultiplier"];
              }
              scores.push([
                stat,
                await Do(
            ...
            let start = (await Do(this.ns, "ns.sleeve.getSleeve", i))["skills"][
              lastupdate.toLowerCase()
            ];
            while (
              start ==
              (await Do(this.ns, "ns.sleeve.getSleeve", i))["skills"][
                lastupdate.toLowerCase()
              ]
            ) {
    ...
    while (-54000 < (await Do(this.ns, "ns.heart.break"))) {
      await this.ns.asleep(10000);
      this.log(
        "Homiciding, Karma: " + (await Do(this.ns, "ns.heart.break")).toString()
      );
    }
    this.Game.Hacknet.goal = "Sell for Money";
  ...
  async trainCombatStatsUpTo(goal, withSleeves = false, halfdexagi = false) {
    ...
    for (let stat of ["Strength", "Defense", "Dexterity", "Agility"]) {
      for (let i = 0; i < (await this.numSleeves); i++) {
        if (
          (halfdexagi && ["Dexterity", "Agility"].includes(stat)
            ? goal / 4
            : goal) >
          (await Do(this.ns, "ns.sleeve.getSleeve", i)).skills[
            stat.toLowerCase()
  ...
  async bbCombatSort() {
    return (async () => {
      try {
        ...
        sleeves = sleeves.sort(
          (b, a) =>
            (100 - sleevestats[a].shock) *
              sleevestats[a].skills.strength *
              sleevestats[a].skills.defense *
              sleevestats[a].skills.dexterity *
              sleevestats[a].skills.agility -
            (100 - sleevestats[b].shock) *
              sleevestats[b].skills.strength *
              sleevestats[b].skills.defense *
              sleevestats[b].skills.dexterity *
  ...
  async bbCombatAugs() {
    ...
    sleeves = sleeves.sort(
      (a, b) =>
        sleevestats[a].strength *
          sleevestats[a].defense *
          sleevestats[a].dexterity *
          sleevestats[a].agility -
        sleevestats[b].strength *
          sleevestats[b].defense *
          sleevestats[b].dexterity *
          sleevestats[b].agility
    );
    for (let i of sleeves) {
      let augs = (
        await Do(this.ns, "ns.sleeve.getSleevePurchasableAugs", i)
      ).map((x) => x.name);
      let augstats = await DoAll(
        this.ns,
      ...
      augs = augs.filter(
        (x) =>
          augstats[x].strength > 1 ||
          augstats[x].strength_exp > 1 ||
          augstats[x].defense > 1 ||
          augstats[x].defense_exp > 1 ||
          augstats[x].dexterity > 1 ||
          augstats[x].dexterity_exp > 1 ||
          augstats[x].agility > 1 ||
          augstats[x].agility_exp > 1
      );
      augs = augs.sort(
        (a, b) =>
          -augstats[a].strength *
            augstats[a].strength_exp *
            augstats[a].defense *
            augstats[a].defense_exp *
            augstats[a].dexterity *
            augstats[a].dexterity_exp *
            augstats[a].agility *
            augstats[a].agility_exp +
          augstats[b].strength *
            augstats[b].strength_exp *
            augstats[b].defense *
            augstats[b].defense_exp *
            augstats[b].dexterity *
            augstats[b].dexterity_exp *
            augstats[b].agility *
            augstats[b].agility_exp
      );
