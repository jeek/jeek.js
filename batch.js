...
//	}
    ns.write("/jeek/hack.js","export async function main(ns) { await ns.hack(ns.args[0])}", 'w');
    ns.write("/jeek/grow.js","export async function main(ns) { await ns.grow(ns.args[0])}", 'w');
    ns.write("/jeek/weaken.js","export async function main(ns) { await ns.weaken(ns.args[0])}", 'w');
	...
	var growThreadsNeeded = Math.ceil((1 + ratio) * ns.growthAnalyze(ns.args[0], 1 / (1 - ratio * .99999)));
	...
	while (true) { //ns.getServerMoneyAvailable(ns.args[0]) > Math.max(5, ns.getServerMaxMoney(ns.args[0]) * (1 - 2 * ratio))) {
		if ((ns.getServerMoneyAvailable <= 1) || ns.getServerMinSecurityLevel(ns.args[0]) * 5 < ns.getServerSecurityLevel(ns.args[0])) {
		...
		while (ns.getPlayer()['hacking'] > startlevel) {
			...
			await ns.sleep(1);
			if ((ns.getServerMoneyAvailable(ns.args[0]) == ns.getServerMaxMoney(ns.args[0])) && (ns.getServerMinSecurityLevel(ns.args[0]) == ns.getServerSecurityLevel(ns.args[0]))) {
				if (trustForm) {
					var hackThreadsNeeded = Math.ceil(ratio / (ns.hackAnalyze(ns.args[0])));
					var weaken1ThreadsNeeded = Math.ceil(hackThreadsNeeded / 25.0);
					var growThreadsNeeded = Math.ceil((1 + ratio) * ns.growthAnalyze(ns.args[0], 1 / (1 - ratio * .99999)));
