...
export class StockMarket {
  constructor(ns, Game) {
    ...
    if (ns.flags(cmdlineflags)["logbox"]) {
      this.log =
        this.Game.sidebar.querySelector(".stockbox") ||
        this.Game.createSidebarItem("Stocks", "", "S", "stockbox");
      this.display = this.Game.sidebar
        .querySelector(".stockbox")
  ...
  async Start() {
    ...
    while (true) {
      ...
      for (let stock of symbols) {
        ...
        this.myvol[stock] = Math.max(
          this.myvol[stock],
          newprice / this.myhist[stock][this.myhist[stock].length - 1] - 1,
          this.myhist[stock][this.myhist[stock].length - 1] / newprice - 1
        );
  ...
  async displayBoxUpdate() {
    while (this.ns.flags(cmdlineflags)["logbox"]) {
      ...
      result +=
        "<TR><TH>sym</TH><TH>position</TH><TH>profit</TH><TH>value</TH></TR>";
      let market = await this.market;
      let keys = Object.keys(market).sort(
        (a, b) => market[b].profit - market[a].profit
      );
      for (let stock of keys) {
        if (
          market[stock]["position"][0] > 0 ||
          market[stock]["position"][2] > 0
        ) {
          result += "<TR><TD>" + stock + "</TD>";
          let pos = [];
          if (market[stock]["position"][0] > 0) {
            pos.push(
              "<RIGHT>" + market[stock]["position"][0].toString() + " L</RIGHT>"
            );
          }
          if (market[stock]["position"][2] > 0) {
            pos.push(
              "<RIGHT>" + market[stock]["position"][2].toString() + " S</RIGHT>"
            );
          }
          result += "<TD ALIGN=RIGHT>" + pos.join("<BR>") + "</TD>";
          result +=
            "<TD ALIGN=RIGHT>" +
            jFormat(market[stock]["profit"], "$") +
            "</TD>";
          result +=
            "<TD ALIGN=RIGHT>" +
            jFormat(market[stock]["value"], "$") +
            "</TD></TR>";
        }
      }
      result += "</TABLE>";
  ...
  async value(stock) {
    ...
    return (
      (await Do(this.ns, "ns.stock.getSaleGain", stock, pos[0], "Long")) +
      (await Do(this.ns, "ns.stock.getSaleGain", stock, pos[2], "Short"))
    );
  }
  async profit(stock) {
    let pos = await this.position(stock);
  ...
  server(stock) {
    if (Object.keys(stockMapping).includes(stock)) return stockMapping[stock];
    return null;
  }
  async stockData(stock) {
    ...
    answer["shortsalevalue"] = await Do(
      this.ns,
      "ns.stock.getSaleGain",
      stock,
      answer["position"][2],
    ...
    answer["profit"] =
      answer["longsalevalue"] +
      answer["shortsalevalue"] -
      answer["position"][0] * answer["position"][1] -
      answer["position"][2] * answer["position"][3];
  ...
  get market() {
    return (async () => {
      try {
        let answer = {};
        let symbols = await this.symbols;
        Object.entries(stockSymbolToCompany).map(
          (x) => (answer[x[0]] = { company: x[1] })
        );
        Object.entries(
          await DoAll(this.ns, "ns.stock.getPosition", symbols)
        ).map((x) => (answer[x[0]]["position"] = x[1]));
        Object.entries(await DoAll(this.ns, "ns.stock.getPrice", symbols)).map(
          (x) => (answer[x[0]]["price"] = x[1])
        );
        Object.entries(
          await DoAll(this.ns, "ns.stock.getAskPrice", symbols)
        ).map((x) => (answer[x[0]]["askprice"] = x[1]));
        Object.entries(
          await DoAll(this.ns, "ns.stock.getBidPrice", symbols)
        ).map((x) => (answer[x[0]]["bidprice"] = x[1]));
        if (await Do(this.ns, "ns.stock.has4SDataTIXAPI", "")) {
          Object.entries(
            await DoAll(this.ns, "ns.stock.getVolatility", symbols)
          ).map((x) => (answer[x[0]]["volatility"] = x[1]));
          Object.entries(
            await DoAll(this.ns, "ns.stock.getForecast", symbols)
          ).map((x) => (answer[x[0]]["forecast"] = x[1]));
        } else {
        ...
        Object.entries(
          await DoAllComplex(
            this.ns,
            "ns.stock.getSaleGain",
            symbols.map((x) => [x, answer[x]["position"][0], "Long"])
          )
        )
          .map((x) => [x[0].split(",")[0], x[1]])
          .map((x) => (answer[x[0]]["longsalevalue"] = x[1]));
        Object.entries(
          await DoAllComplex(
            this.ns,
            "ns.stock.getSaleGain",
            symbols.map((x) => [x, answer[x]["position"][2], "Short"])
          )
        )
          .map((x) => [x[0].split(",")[0], x[1]])
          .map((x) => (answer[x[0]]["shortsalevalue"] = x[1]));
        symbols.map(
          (x) =>
            (answer[x]["value"] =
              answer[x]["longsalevalue"] + answer[x]["shortsalevalue"])
        );
        symbols.map(
          (x) =>
            (answer[x]["profit"] =
              answer[x]["value"] -
              answer[x]["position"][0] * answer[x]["position"][1] -
              answer[x]["position"][2] * answer[x]["position"][3])
        );
        symbols.map(
          (x) =>
            (answer[x]["server"] = stockMapping[x] ? stockMapping[x] : null)
        );
        return answer;
      } catch (e) {
        this.ns.tprint(e);
  ...
  async createDisplay() {
    ...
    eval("window").listenUpStonk = (message) => {
      globalThis.stockQueue.push(message);
    };
  ...
  async updateDisplay() {
      ...
      for (let stock of Object.keys(data)) {
        ...
        if (data[stock]["position"][2] > 0) {
          await Do(
            ...
            "ns.stock.sellShort",
            stock,
            data[stock]["position"][2]
          );
        }
      }
    }
    let sourcefiles = [];
    let servermoneyavailable = await DoAll(
      this.ns,
      "ns.getServerMoneyAvailable",
      Object.values(stockMapping)
    );
    let servermaxmoney = await DoAll(
      this.ns,
      "ns.getServerMaxMoney",
      Object.values(stockMapping)
    );
    let serverminsecuritylevel = await DoAll(
      this.ns,
      "ns.getServerMinSecurityLevel",
      Object.values(stockMapping)
    );
    let serversecuritylevel = await DoAll(
      this.ns,
    ...
    for (let stock of Object.keys(data)) {
      ...
      myupdate += td(
        jFormat(data[stock]["price"], "$") +
          "<BR><SMALL>" +
          jFormat(data[stock]["askprice"], "$") +
          "<BR>" +
          jFormat(data[stock]["bidprice"], "$"),
        "RIGHT"
      );
      if (data[stock]["position"][0] > 0) {
        myupdate += td(
          jFormat(data[stock]["position"][0]) +
            "<BR><SMALL>" +
            jFormat(data[stock]["position"][1], "$") +
            (data[stock]["longsalevalue"] != 0
              ? '<BR><a href="#" onClick=\'window.opener.listenUpStonk("Do(this.ns, \\"ns.stock.sellStock\\", \\"' +
                stock +
                '\\", ' +
                data[stock]["position"][0] +
                ")\")'>" +
                jFormat(data[stock]["longsalevalue"], "$") +
                "</A>"
              : ""),
          "RIGHT"
        );
      ...
      if (bn == 8 || sourcefiles.filter((x) => x.n == 8 && x.lvl >= 2)) {
        if (data[stock]["position"][2] > 0) {
          myupdate += td(
            jFormat(data[stock]["position"][2]) +
              "<BR><SMALL>" +
              jFormat(data[stock]["position"][3], "$") +
              (data[stock]["shortsalevalue"] != 0
                ? "<BR>" +
                  '<a href="#" onClick=\'window.opener.listenUpStonk("Do(this.ns, \\"ns.stock.sellShort\\", \\"' +
                  stock +
                  '\\", ' +
                  data[stock]["position"][2] +
                  ")\")'>" +
                  jFormat(data[stock]["shortsalevalue"], "$") +
                  "</A>"
                : ""),
            "RIGHT"
          );
      ...
      if (data[stock]["profit"] != 0) {
        myupdate += td(
          (data[stock]["profit"] < 0
            ? "<FONT COLOR='" + this.ns.ui.getTheme()["error"] + "'>"
            : "") + jFormat(data[stock]["profit"], "$"),
          "RIGHT"
        );
      } else {
        myupdate += td("&nbsp;");
      ...
      myupdate += td(
        this.ns.nFormat(100 * data[stock]["volatility"], "0.00"),
        "RIGHT"
      );
      ...
      if (Object.keys(stockMapping).includes(stock)) {
        ...
        myupdate +=
          "$$$: " +
          Math.floor(
            (100 * servermoneyavailable[stockMapping[stock]]) /
              servermaxmoney[stockMapping[stock]]
          ).toString() +
          "%<BR>";
        myupdate +=
          "Sec: " +
          Math.floor(
            (100 * serverminsecuritylevel[stockMapping[stock]]) /
              serversecuritylevel[stockMapping[stock]]
          ).toString() +
          "%</TD>";
      } else {
        myupdate += td("&nbsp;");
      ...
      myupdate += "</TR>";
      if (
        !this.ns.flags(cmdlineflags)["stockfilter"] ||
        data[stock]["position"][0] + data[stock]["position"][2] > 0
      ) {
    ...
    update =
      "<H1>Holdings: " +
      jFormat(await this.portfolioValue, "$") +
      (totalProfit < 0
        ? "<FONT COLOR='" + this.ns.ui.getTheme()["error"] + "'>"
        : "<FONT>") +
      " (Profit: " +
      jFormat(totalProfit, "$") +
      ")</FONT></H1> " +
      '<a href="#" onClick=\'window.opener.listenUpStonk("this.liquidate=!this.liquidate")\'>' +
      (this.liquidate
        ? "Liquidating"
        : "<FONT COLOR='" +
          this.ns.ui.getTheme()["error"] +
          "'>Click to liquidate</FONT>") +
      "</A>" +
      "<BR>" +
      update;
