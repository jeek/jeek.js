...
export class Grafting {
  constructor(ns, game) {
    ...
    if (ns.flags(cmdlineflags)["logbox"]) {
      this.log =
        this.game.sidebar.querySelector(".graftbox") ||
        this.game.createSidebarItem("Grafting", "", "G", "graftbox");
  ...
  async checkIn(type = "Hacking", force = false) {
    ...
    if (
      ...
      !(await Do(this.ns, "ns.singularity.isFocused", ""))
    ) {
      ...
      switch (type) {
        case "Combat":
          auglist = auglist.sort(
            (a, b) =>
              augs[b].agility_exp *
                augs[b].agility *
                augs[b].defense_exp *
                augs[b].defense *
                augs[b].dexterity_exp *
                augs[b].dexterity *
                augs[b].strength_exp *
                augs[b].strength -
              augs[a].agility_exp *
        ...
        case "Charisma":
          auglist = auglist.sort(
            (a, b) =>
              augs[b].charisma_exp * augs[b].charisma -
              augs[a].charisma_exp * augs[a].charisma
          );
          break;
        case "Hacking":
          auglist = auglist.sort(
            (a, b) =>
              augs[b].hacking_grow *
                augs[b].hacking_speed *
                augs[b].hacking ** 2 *
                augs[b].hacking_exp ** 2 *
                augs[b].faction_rep ** 0.1 -
              augs[a].hacking_grow *
                augs[a].hacking ** 2 *
                augs[a].hacking_exp ** 2 *
                augs[a].hacking_speed *
                augs[a].faction_rep ** 0.1
          );
          break;
      }
      let currentaugs = await Do(
        this.ns,
      ...
      for (let special of [
        ...
        if (
          (
            await Do(this.ns, "ns.grafting.getGraftableAugmentations", "")
          ).includes(special)
        ) {
          if (
            (await Do(
