...
class Division extends CorpBaseClass {
  constructor(ns, Corp, industry, settings = {}) {
    ...
    this.Optimizer = new WarehouseOptimizer(
      ...[
        ...
        Object.keys(this.c.getIndustryData(this.industry)).includes(factor)
          ? this.c.getIndustryData(this.industry)[factor]
          : 0
      ),
      ns
    );
  ...
  get name() {
    return this.c
      ...
      .filter((x) => x[1] == this.industry)[0][0];
  }
  get cities() {
  ...
  async getHappy() {
    while (
      !this.c
        .getCorporation()
        .divisions.map((x) => this.c.getDivision(x).type)
  ...
  async Research(queue) {
    while (
      queue
        .map((x) => this.c.hasResearched(this.name, x))
        .reduce((a, b) => a && b) == false
    ) {
      let cost =
        queue
          .filter((x) => !this.c.hasResearched(this.name, x))
          .map((x) => this.c.getResearchCost(this.name, x))
          .reduce((a, b) => a + b, 0) * 2;
