...
export class Hacknet {
  constructor(ns, Game, goal = "") {
    ...
    if (ns.flags(cmdlineflags)["logbox"]) {
      this.log =
        this.Game.sidebar.querySelector(".hacknetbox") ||
        this.Game.createSidebarItem("Hacknet", "", "H", "hacknetbox");
      this.display = this.Game.sidebar
        .querySelector(".hacknetbox")
  ...
  async displayUpdate() {
    while (this.ns.flags(cmdlineflags)["logbox"]) {
      ...
      if ((await Do(this.ns, "ns.hacknet.numNodes")) > 0) {
        ...
        result +=
          "<TH>id</TH><TH>level</TH><TH>ram</TH><TH>cores</TH><TH>cache</TH><TH>prod/s</TH></TR>";
        let rowData = {};
      ...
      let header =
        "<center><h2>hashes: " +
        jFormat(await Do(this.ns, "ns.hacknet.numHashes")) +
        "/" +
        jFormat(await Do(this.ns, "ns.hacknet.hashCapacity")) +
        "</h2>prod: " +
        jFormat(totalProd) +
        "/s</center><br>";
      this.display.innerHTML =
        ...
        result +
        "next node: " +
        jFormat(await Do(this.ns, "ns.hacknet.getPurchaseNodeCost"), "$");
    ...
    while (true) {
      ...
      if (this.goal == "Sell for Money") {
        await Do(
          ...
          this.goal,
          "",
          Math.floor((await Do(this.ns, "ns.hacknet.numHashes", "")) / 4)
        );
      ...
      //			}
      if (
        (await Do(this.ns, "ns.getMoneySources")).sinceInstall
          .hacknet_expenses >= -1e9 ||
        0 <=
          (await Do(this.ns, "ns.getMoneySources")).sinceInstall["hacknet"] +
            (await Do(this.ns, "ns.getMoneySources")).sinceInstall
              .hacknet_expenses
      ) {
        let didSomething = true;
        let mults = (await Do(this.ns, "ns.getPlayer", "")).mults
          .hacknet_node_money;
        while (didSomething) {
          didSomething = false;
          let shoppingCart = [
            [
              (await Do(this.ns, "ns.hacknet.getPurchaseNodeCost")) /
                this.ns.formulas.hacknetServers.hashGainRate(1, 0, 1, 1, mults),
              await Do(this.ns, "ns.hacknet.getPurchaseNodeCost"),
          ...
          for (let i = 0; i < (await Do(this.ns, "ns.hacknet.numNodes")); i++) {
            ...
            shoppingCart.push([
              this.ns.formulas.hacknetServers.ramUpgradeCost(
                current.ram,
                1,
                mults.hacknet_node_ram_cost
              ) /
                (this.ns.formulas.hacknetServers.hashGainRate(
                  current.level,
                  0,
                  current.ram * 2,
                  current.cores,
            ...
            shoppingCart.push([
              this.ns.formulas.hacknetServers.coreUpgradeCost(
                current.cores,
                1,
            ...
            shoppingCart.push([
              this.ns.formulas.hacknetServers.levelUpgradeCost(
                current.level,
                1,
                mults.hacknet_node_core_cost
              ) /
          ...
          shoppingCart = shoppingCart.sort((a, b) => {
            return a[0] - b[0];
          });
        ...
        let done = false;
        while (
          (await Do(this.ns, "ns.hacknet.numHashes")) * 2 >
            (await Do(this.ns, "ns.hacknet.hashCapacity")) &&
          !done
        ) {
          done = true;
          let minimum = 1e308;
          ...
          for (let i = 0; i < (await Do(this.ns, "ns.hacknet.numNodes")); i++) {
            if (
              this.ns.formulas.hacknetServers.cacheUpgradeCost(
                (await Do(this.ns, "ns.hacknet.getNodeStats", i)).cache,
                1,
                mults.hacknet_node_cache_cost
              ) < minimum
            ) {
              answer = i;
              minimum = this.ns.formulas.hacknetServers.cacheUpgradeCost(
                (await Do(this.ns, "ns.hacknet.getNodeStats", i)).cache,
                1,
                mults.hacknet_node_cache_cost
              );
          ...
          if (
            answer >= 0 &&
            (await Do(this.ns, "ns.hacknet.upgradeCache", answer))
          ) {
            done = false;
          }
        }
        ...
        if (this.goal == "Sell for Money") {
          let poof = Math.floor(
            (await Do(this.ns, "ns.hacknet.numHashes", "")) / 4
          );
          await Do(
            this.ns,
            "ns.hacknet.spendHashes",
