...
export class Gang {
  constructor(ns, Game, settings = {}) {
    ...
    if (this.ns.flags(cmdlineflags)["logbox"]) {
      this.log =
        this.Game.sidebar.querySelector(".gangbox") ||
        this.Game.createSidebarItem("Gang", "", "G", "gangbox");
  ...
  async displayBoxUpdate() {
    ...
    while (this.ns.flags(cmdlineflags)["logbox"]) {
      ...
      let memberNames = Object.keys(memberData).sort(
        (a, b) => memberData[b].earnedRespect - memberData[a].earnedRespect
      );
      ...
      for (let member of memberNames) {
        result +=
          ...
          member +
          "</TD><TD>" +
          jFormat(memberData[member].earnedRespect) +
          "</TD><TD>" +
          ...
          "</TD><TD>" +
          memberData[member].agi +
          "</TD><TD>" +
          memberData[member].cha +
          "</TD><TD>" +
          memberData[member].task +
          "</TD></TR>";
      }
      result += "</TABLE>";
  ...
  async ["getEquipmentNames"]() {
    this.equipnames =
      this.equipnames ?? (await Do(this.ns, "ns.gang.getEquipmentNames"));
    return this.equipnames;
  }
  async ["getEquipmentStats"](equipName) {
    this.equipstats[equipName] =
      this.equipstats[equipName] ??
      (await Do(this.ns, "ns.gang.getEquipmentStats", equipName));
  ...
  async ["getTaskNames"]() {
    this.tasknames =
      this.tasknames ?? (await Do(this.ns, "ns.gang.getTaskNames"));
    return this.tasknames;
  }
  async ["getTaskStats"](name) {
    this.taskstats[name] =
      this.taskstats[name] ?? (await Do(this.ns, "ns.gang.getTaskStats", name));
  ...
  async recruitMembers(names) {
    ...
    if (usedNames.length == 12) return;
    while (
      await Do(
        this.ns,
        "ns.gang.recruitMember",
        names.filter((x) => !usedNames.includes(x))[0]
      )
    ) {
      this.log(
        "New member " +
