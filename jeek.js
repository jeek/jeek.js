...
export class Bladeburner {
	...
	async updateDisplay() {
		...
		answer += "</TR>";
		if (0 < await (this.game.Sleeves.numSleeves)) {
			answer += "<TR VALIGN=TOP>" + td("Sleeves");
			for (let city of CITIES.sort()) {
				if (city != mycity) {
					answer += "<TD>";
					let wildcard = true;
...
/** @param {NS} ns */
export async function main(ns) {
    await ns.wget("https://raw.githubusercontent.com/jeek/jeek.js/dev/main.js", "/temp/main.js");
    let imports = ["main.js"];
    let output = "";
    for (let i = 0; i < imports.length; i++) {
        let file = imports[i];
        ...
        for (let line of content) {
            if (line.slice(0, 6) == "import") {
                ...
                if (!imports.includes(newImport)) {
                    ns.tprint(newImport);
                    imports.push(newImport);
                    await ns.wget("https://raw.githubusercontent.com/jeek/jeek.js/dev/" + newImport, "/temp/" + newImport);
                }
            }
        }
    }
