...
export class Player {
  ...
  async Gym(stat, gymName = "Powerhouse Gym", focus = false) {
    ...
    if (
      ["Powerhouse Gym", "Iron Gym"].includes(gymName) &&
      (await this.city) != "Sector-12"
    ) {
    ...
    if (
      ["Crush Fitness Gym", "Snap Fitness Gym"].includes(gymName) &&
      (await this.city) != "Aevum"
    ) {
    ...
    if (
      ["Millenium Fitness Gym"].includes(gymName) &&
      (await this.city) != "Volhaven"
    ) {
  ...
  async trainCombatStatsUpTo(goal, withSleeves = false) {
    ...
    for (let stat of ["Strength", "Defense", "Dexterity", "Agility"]) {
      if (withSleeves && (await this.game.Sleeves.numSleeves) > 0) {
        if (
          goal > (await Do(this.ns, "ns.getPlayer")).skills[stat.toLowerCase()]
        ) {
