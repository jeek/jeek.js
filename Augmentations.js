...
export class Augmentations {
  ...
  async createDisplay() {
    this.augWindow = await makeNewWindow(
      "Augmentations",
      this.ns.ui.getTheme()
    );
    this.augs = {};
    let graftableaugs = await Do(
      this.ns,
      ...
      for (let aug of await Do(
        ...
        if (!Object.keys(this.augs).includes(aug)) {
          ...
          if (graftableaugs.includes(aug)) {
            this.augs[aug]["graftprice"] = await Do(
              this.ns,
              "ns.grafting.getAugmentationGraftPrice",
              aug
            );
            this.augs[aug]["grafttime"] = await Do(
              this.ns,
  ...
  async updateDisplay() {
    ...
    for (let aug of Object.keys(this.augs).sort(
      (a, b) => this.augs[a]["rep_req"] - this.augs[b]["rep_req"]
    )) {
      let skip = false;
      if (
        this.augs[aug].factions.includes("Church of the Machine God") &&
        ownedaugs.length > 0 &&
        !ownedaugs.includes("Stanek's Gift - Genesis")
      ) {
        skip = true;
      }
      if (
        this.augs[aug].factions.includes("Bladeburners") &&
        !(await Do(this.ns, "ns.getPlayer")).factions.includes("Bladeburners")
      ) {
        skip = true;
      }
      if (
        this.augs[aug].factions.includes("Shadows of Anarchy") &&
        !(await Do(this.ns, "ns.getPlayer")).factions.includes(
          "Shadows of Anarchy"
        )
      ) {
        skip = true;
      ...
      if (!skip) {
        ...
        myupdate +=
          td(aug) + td(jFormat(this.augs[aug]["price"], "$"), "RIGHT");
        myupdate +=
          "<TD ALIGN=RIGHT>" +
          jFormat(this.augs[aug]["rep_req"]) +
          "<BR><TABLE BORDER=0 CELLPADDING=0 CELLSPACING=0>";
        let nothing = [];
        for (let faction of this.augs[aug]["factions"].sort(
          (a, b) => factrep[b] - factrep[a]
        )) {
          if (factrep[faction] > 0) {
            myupdate += tr(
              td(FACTIONS[faction]["abbrev"] + "&nbsp;", "RIGHT") +
                td(jFormat(factrep[faction]), "RIGHT")
            );
          } else {
        ...
        try {
          myupdate += td(
            timeFormat(Math.floor(this.augs[aug]["grafttime"] / 1000 + 0.5)),
            "RIGHT"
          );
        } catch {
          myupdate += td("&nbsp;");
