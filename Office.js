...
class Office extends CorpBaseClass {
  ...
  async Start() {
    ...
    if (
      this.size == 3 &&
      this.c.getOffice(this.Division.name, this.name).employeeJobs[
        "Unassigned"
      ] == 3
    )
  ...
  async Hire(roles) {
    ...
    while (this.size < total) {
      if (
        this.c.getOfficeSizeUpgradeCost(this.Division.name, this.name, 3) <=
        this.funds
      ) {
      ...
      for (let job of [
        ...
        (a, b) =>
          this.c.getOffice(this.Division.name, this.name).employeeJobs[a] -
          roles[a] -
          this.c.getOffice(this.Division.name, this.name).employeeJobs[b] +
          roles[b]
      )) {
        while (
          this.employees < this.size &&
          this.getOffice.employeeJobs[job] < roles[job]
        ) {
    ...
    let good = true;
    for (let job of Object.keys(roles).sort(
      (a, b) =>
        this.c.getOffice(this.Division.name, this.name).employeeJobs[a] -
        roles[a] -
        this.c.getOffice(this.Division.name, this.name).employeeJobs[b] +
  ...
  async coffeeparty() {
    ...
    while (true) {
      ...
      if (this.getOffice.employees > 0) {
        if (
          this.getOffice.avgEne < this.settings.minEnergy &&
          this.getOffice.employees *
            this.c.getConstants().coffeeCostPerEmployee <
            this.funds
        ) {
          this.c.buyCoffee(this.Division.name, this.name);
        }
        if (
          (this.getOffice.avgHap < this.settings.minHappy ||
            this.getOffice.avgMor < this.settings.minMorale) &&
          this.getOffice.employees *
            this.c.getConstants().coffeeCostPerEmployee <
            this.funds
        ) {
