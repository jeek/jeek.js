...
class Warehouse extends CorpBaseClass {
  ...
  async FF(mysize = -1, maintaining = false) {
    ...
    await this.getAPI();
    if (mysize == -1) mysize = this.size;
    mysize = Math.floor(mysize);
    if (mysize < 0) mysize = 0;
    let mymats = [0, 0, 0, 0, 0];
    for (let twice of [0, 1]) {
      mymats = this.Optimizer.optimize(
        mysize * [0.5, 0.55, 0.55, 0.57, 0.57, 0.57][this.round]
      );
      ...
      for (let material of ["Hardware", "AI Cores", "Robots", "Real Estate"]) {
        ...
        if (
          this.c.hasResearched(this.Division.name, "Bulk Purchasing") ||
          true
        ) {
          if (
            this.c.getMaterial(this.Division.name, this.name, material).qty <=
            mymats[matIndex]
          ) {
            let qty =
              mymats[matIndex] -
            ...
            while (
              qty > 0 &&
              qty *
                this.c.getMaterial(this.Division.name, this.name, material)
                  .cost >
                this.funds
            ) {
              qty = Math.floor(qty * 0.9);
            }
            if (qty > 0) {
          ...
          if (
            (!Object.keys(this.industryData).includes("producedMaterials") ||
              !Object.keys(this.industryData.producedMaterials).includes(
                material
              )) &&
            !maintaining
          ) {
  ...
  async upgradeSize(size, growafterwards = false) {
    ...
    while (this.warehouseSize < size) {
      if (
        this.c.getUpgradeWarehouseCost(this.Division.name, this.name) <=
        this.funds
      ) {
  ...
  async enableSmartSupply() {
    while (!this.c.hasUnlockUpgrade("Smart Supply")) {
      ...
      if (
        this.c.getUnlockUpgradeCost("Smart Supply") <= this.funds &&
        !this.c.hasUnlockUpgrade("Smart Supply")
      ) {
    ...
    while (true) {
      ...
      for (let product of this.Division.getDivision.products) {
        if (!this.c.hasResearched(this.Division.name, "Market-TA.II")) {
          if (
            this.c.getProduct(this.Division.name, this.name, product)
            ...
            if (!Object.keys(this.pricing).includes(product)) {
              ...
              if (this.pricing[product].phase == 3) {
                if (
                  this.c.getProduct(this.Division.name, this.name, product)
                    .prod <
                  this.c.getProduct(this.Division.name, this.name, product).sell
                ) {
                  this.pricing[product].x_min =
                    this.pricing[product].x_min + 1 >= 1
                      ? this.pricing[product].x_min + 1
                      : 1;
          ...
          if (
            !Object.keys(this.pricing).includes(product) ||
  ...
  async MaintainWarehouse() {
    ...
    for (let material of Object.keys(this.industryData.requiredMaterials)) {
      productSize +=
        this.industryData.requiredMaterials[material] *
        this.c.getMaterialData(material).size;
    }
    while (true) {
      while (this.c.getCorporation().state != "SALE") await this.ns.asleep(100);
      let sizes = [0, 0, 0]; // Incoming, Outgoing, Qty
      let prod = Object.keys(this.industryData).includes("producedMaterials")
        ? this.industryData.producedMaterials
        : [];
      for (let prodmat of prod) {
        let mysize = this.c.getMaterialData(prodmat).size;
        let myprod = this.c.getMaterial(
          this.Division.name,
          this.name,
          prodmat
        ).prod;
        ...
        if (
          !["Hardware", "AI Cores", "Robots", "Real Estate"].includes(prodmat)
        )
          sizes[2] +=
            mysize *
            this.c.getMaterial(this.Division.name, this.name, prodmat).qty;
        for (let material of Object.keys(this.industryData.requiredMaterials)) {
          sizes[0] +=
            myprod *
            this.c.getMaterialData(material).size *
            this.industryData.requiredMaterials[material];
          if (
            !["Hardware", "AI Cores", "Robots", "Real Estate"].includes(
              material
            )
          )
            sizes[2] +=
      ...
      let products = Object.keys(this.Division.getDivision).includes("products")
        ? this.Division.getDivision.products
        : [];
      for (let product of products) {
        let myprod = this.c.getProduct(
          this.Division.name,
          this.name,
          product
        ).prod;
        ...
        sizes[2] +=
          productSize *
          this.c.getProduct(this.Division.name, this.name, product).qty;
        for (let material of Object.keys(this.industryData.requiredMaterials)) {
          sizes[0] +=
            myprod *
            this.c.getMaterialData(material).size *
          ...
          if (
            (!"AI Cores",
            ["Hardware", "Real Estate", "Robots"].includes(material))
          )
            sizes[2] +=
              this.c.getMaterialData(material).size *
              this.c.getMaterial(this.Division.name, this.name, material).qty;
      ...
      let targetsize =
        this.c.getWarehouse(this.Division.name, this.name).size -
        (sizes[0] > sizes[1] ? sizes[0] : sizes[1]) -
        sizes[2];
      let sizecheck = this.Optimizer.optimize(
        targetsize / [0.5, 0.7, 0.55, 0.57, 0.57, 0.57][this.round]
      );
      if (
        sizecheck[2] >
        this.c.getMaterial(this.Division.name, this.name, "Real Estate").qty
      ) {
        await this.FF(targetsize, true);
      } else {
        if (
          this.c.getWarehouse(this.Division.name, this.name).sizeUse >
          this.c.getWarehouse(this.Division.name, this.name).size * 0.95
        ) {
          if (
            this.funds >
            this.c.getUpgradeWarehouseCost(this.Division.name, this.name)
  ...
  async Exporty() {
    while (!this.c.hasUnlockUpgrade("Export")) await this.WaitOneLoop();
    await this.getAPI();
    while (true) {
      while (this.c.getCorporation().state != "SALE") {
        await this.ns.asleep(100);
      ...
      for (let material of Object.keys(this.industryData.requiredMaterials)) {
        if (!Object.keys(providers).includes(material))
          providers[material] = this.c
            .getConstants()
            .industryNames.map((x) => [x, this.c.getIndustryData(x)])
            .filter((x) => Object.keys(x[1]).includes("producedMaterials"))
            .map((x) => [x[0], x[1].producedMaterials])
            .filter((x) => x[1].includes(material))
            .map((x) => x[0])
            .flat()
            .map((x) =>
              this.c
                .getCorporation()
                .divisions.filter((y) => this.c.getDivision(y).type == x)
            )
            .flat();
        if (providers[material].length > 0) {
          let needed = 0;
          if (
            this.c.getIndustryData(this.Division.industry).producedMaterials
          ) {
            for (let outputmat of this.industryData.producedMaterials) {
              needed +=
                this.c.getMaterial(this.Division.name, this.name, outputmat)
                  .prod *
                this.c.getIndustryData(this.Division.industry)
                  .requiredMaterials[material];
            }
          }
          if (this.Division.getDivision.products.length > 0) {
            for (let product of this.c
              .getDivision(this.Division.name)
              .products.filter(
                (x) =>
                  this.c.getProduct(this.Division.name, this.name, x)
                    .developmentProgress >= 100
              )) {
              needed +=
                this.c.getProduct(this.Division.name, this.name, product).prod *
                this.industryData.requiredMaterials[material];
            }
          }
          try {
              ...
              Math.floor(
                needed -
                  this.c.getMaterial(this.Division.name, this.name, material)
                    .qty /
                    10 /
                    providers[material].length
              ),
            ].reduce((a, b) => (a > b ? a : b));
            for (let provider of providers[material]) {
              if (this.c.getDivision(provider).cities.includes(this.name)) {
                let currentExports = this.c
                  .getMaterial(provider, this.name, material)
                  .exp.filter(
                    (x) => x.loc == this.name && x.div == this.Division.name
                  );
                currentExports.map((x) =>
                  this.c.cancelExportMaterial(
                    provider,
                    this.name,
                    this.Division.name,
                    this.name,
